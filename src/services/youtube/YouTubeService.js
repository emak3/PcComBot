...
class YouTubeService {
    ...
    async getLatestVideos(channelId, maxResults = 5) {
        try {
            ...
            log.error(`YouTube APIエラー (チャンネル: ${channelId}):`, error.message);
            
            if (error.response?.status === 403) {
                log.error('YouTube API クォータ制限またはAPIキーが無効です');
            } else if (error.response?.status === 404) {
                log.error('指定されたチャンネルが見つかりませんでした');
            }
    ...
    async getUploadsPlaylistId(channelId) {
        try {
            ...
            log.error(`アップロードプレイリストID取得エラー:`, error.message);
            throw error;
        }
    }
    ...
    async getVideoDetails(videoId) {
        try {
            ...
            log.error(`動画詳細取得エラー (動画ID: ${videoId}):`, error.message);
            return null;
        }
    }
    ...
    async getChannelInfo(channelId) {
        try {
            ...
            log.error(`チャンネル情報取得エラー:`, error.message);
            throw error;
        }
    }
}
